...
<body>
  ...
  <!-- <th scope="col">WTPNo</th>
        ...
        <?php
         ...
         $search = $_SESSION['wtpsrch'];
         $sql = "SELECT * FROM wtpgen WHERE WTPNO= ?";
            ...
            while ($row = $result->fetch_assoc()) {
                if (!empty($row)) {   
                echo "<h3 style='color:rgb(0, 140, 255); padding-left: 5%;padding-top: 1%;letter-spacing: 1px;'>".$row['WTPNAME']."</h3>";
                }
            }
        ?>
    ...
    // SQL QUERY
    $query = "SELECT * FROM wtpinfo WHERE WTPNO=?;";
    $statement = $conn->prepare($query);
    $statement->bind_param('i',$_SESSION['wtpsrch']);
    $statement->execute();
    
    // FETCHING DATA FROM DATABASE
    $result = $statement->get_result();
    $q = "SELECT * FROM wtpinfo WHERE WTPNO IN (SELECT WTPNO FROM wtpinfo);";
    $st = $conn->prepare($query);
    $st->bind_param('i',$_SESSION['wtpsrch']);
    $st->execute();
    $res = $st->get_result();
    $r = $res->fetch_assoc();
    if(empty($r)){
...
<table class="table table-dark table-striped-columns">
    ...
    ?>
        ...
        <td><?php echo $row['WTPDATE'] ?></td>
        <td><?php echo $row['WTPTIME'] ?></td>
        <td><?php echo $row['WTPWATERIN'] ?></td>
        <td><?php echo $row['WTPWATEROUT'] ?></td>
